...
class DICProcessorAPI:
    ...
    def __init__(self, results_dir: str = "api_results"):
        ...
        self.processing_tasks = {}  # Словарь активных задач
    ...
    async def process_test_async(self, test_id: str, img1: np.ndarray, img2: np.ndarray,
        ...
        print(f"Параметры анализа (среднее окно):")
        print(f"  Размер окна: {subset_size} пикселей")
        print(f"  Шаг анализа: {step} пикселей")
        print(f"  Макс. итераций: {max_iter}")
        
        # Создаем директорию для теста
        test_dir = os.path.join(self.results_dir, test_id)
        Path(test_dir).mkdir(parents=True, exist_ok=True)
        
        # Создаем экземпляр DIC с оптимальными параметрами
        ...
        try:
            ...
            # Постобработка с оптимальным порогом корреляции
            U_filtered, V_filtered = await dic.post_process_displacements(U, V, C, min_correlation=0.4)
            
            # Сохраняем три отдельные картинки в потоке
            ...
            print(f"\nОбработка теста {test_id} завершена успешно!")
            print(f"Время обработки: {processing_time:.2f} секунд")
            print(f"Качество анализа: {results['statistics']['correlation_quality']:.3f}")
    ...
    async def process_test_from_files_async(self, test_id: str, img1_path: str, img2_path: str, 
        ...
        try:
            ...
            print(f"\nИзображения загружены. Размер: {img1.shape}")
            
            # Обработка теста с оптимальными параметрами для реальных изображений
    ...
    async def _load_images_async(self, img1_path: str, img2_path: str) -> Tuple[np.ndarray, np.ndarray]:
        ...
        def load_image_sync(path):
            # Используем matplotlib для загрузки изображений
